...
- **D9 DOGFOOD-VERIFIED IN PRODUCTION 2026-06-12 10:24 ET.** Filed dogfood ticket foreman#286 (D4 calibration clause for CLAUDE.md/planner.md/reviewer.md). Loop ran: Planner → spec PR #287 → Reviewer-spec → 1 fix loop → approved → auto-merge spec → Worker → impl PR #288 → Reviewer-impl approved → loop intentionally parked at `impl-approved` (`auto_merge_impl=False`) → manual `merging-impl` label transition simulated auto-merge-impl path → **D9 retarget fired at 14:21:46 UTC**, base flipped `foreman/issue-286` → `main` → `update_branch` for BEHIND → CI green on new base → **PR #288 squash-merged to main as commit `fe009e16` at 14:24 UTC** → reconciler advanced label to `foreman:done`. Content (CLAUDE.md +6, planner.md +6, reviewer.md +8, spec doc) actually landed on main — NOT orphaned on the spec branch. This is the exact regression D9 was designed to fix, verified end-to-end.
- **D9 close-out picker fix LANDED 2026-06-12 10:40 ET** as PR #289 → commit `873e75ee` on main. During dogfood diagnosis, surfaced that `_pick_pr_for_ticket` (`daemon.py:69-70`) did not include `foreman:merging-plan` or `foreman:merging-impl` in either phase set. Hidden in production by `ADVANCE_LABEL_TO_MERGING_*` keeping the prior `*-approved` label alongside — picker routed via that. Any path that strips the approved label tripped a silent stall (the 75-minute symptom that surfaced during dogfood diagnosis). Two regression tests pin the new contract; defense-in-depth for D9's correctness under any operational state. D9 is now genuinely closed end-to-end.
