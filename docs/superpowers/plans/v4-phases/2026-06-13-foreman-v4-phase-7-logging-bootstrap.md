...
**From Phase 6 (Task 6.5 review):**
- ~~**`Poller` public signature mismatch.** `Poller.__init__` currently annotates `qm: QueueManager` (non-optional), but `Daemon._with_qm` constructs Pollers with `qm=None` then mutates `poller._qm` after the fact. mypy passes today but the signature lies about what's accepted. Fix one of: (a) `qm: QueueManager | None = None` on Poller (preferred — most callers will pass None and let Daemon inject), OR (b) expose `Poller.set_qm(qm)` so Daemon's `_with_qm` doesn't reach into a private attr. Either is one-file change in `poller.py`; daemon.py + tests update with it.~~ **Resolved in Task 7.4** — Poller now accepts `qm: QueueManager | None`, with `tick()` raising RuntimeError if called before a QM is wired. Option (a) taken.
