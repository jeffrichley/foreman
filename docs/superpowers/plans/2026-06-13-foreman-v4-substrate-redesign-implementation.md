...
Eight phases, ~57 tasks, ~13k lines of plan content. Each task is bite-sized + TDD-shaped + commit-bounded. The substrate replacement runs from foundation → observers → states → orchestration → role rewrite → CLI → production wiring → cutover.
...
**Execution:** subagent-driven development (per the spec's recommendation), task-by-task, with continuous execution. Pre-push hook keeps `just check` green. Each task ends with a commit; each phase ends with the gate.
