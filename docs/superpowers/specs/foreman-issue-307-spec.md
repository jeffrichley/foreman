...
- **Single import point.** A new `packages/foreman/tests/test_label_manager.py::test_label_writes_only_go_through_label_manager` enforces that the only modules importing PyGithub's `Issue.add_to_labels` / `Issue.remove_from_labels` / `Issue.set_labels` symbols (by literal grep against `packages/foreman/src/foreman/`) are: `foreman/label_manager.py`, the two writer adapters (`foreman/daemon_host.py` lines 87/93 and `foreman/git_hosts/github.py` lines 183/185 — the thin PyGithub wrappers), and any test files under `packages/foreman/tests/`. Every role module (`roles/planner.py`, `roles/reviewer.py`, `roles/worker.py`, `roles/fixer.py`) and `reconciler/actions.py` are excluded — they MUST route through the manager.
- **Reconciler action handlers route through the manager.** Each label-mutating action handler in `packages/foreman/src/foreman/reconciler/actions.py` is converted to a single `label_manager.transition(...)` call. The specific sites:
  ...
  - `Action.ADVANCE_LABEL_TO_PLAN_APPROVED` (lines 793-805) — was sequential `remove(PLANNING) + add(PLAN_APPROVED)`; becomes one transition. CRITICALLY, writing `PLAN_APPROVED` (QUEUE) MUST strip `MERGING_PLAN` (IN_FLIGHT) automatically via the invariant — this is one of two paths that fixes the #303 rot.
...
- **Role modules route through the manager.** Each `issue.set_labels(*sorted(final_label_set))` site in `roles/{worker,fixer,reviewer}.py` is replaced with one `label_manager.transition(...)` call. The role's outcome branch still computes the `add` / `remove` sets (the role's verdict — `removed_foreman` / `added_foreman` in today's code) and passes them in; the manager handles the read-update-write cycle and invariants. Sites:
  ...
  - The four `lambda: bound_issue.add_to_labels(TERMINAL_BLOCKING_LABEL)` callbacks for `handle_unhandled_role_exception` (reviewer.py:510, fixer.py:489, fixer.py:623, worker.py:674, worker.py:862) — convert each to invoke the manager. The exception path is the one place where atomicity is most fragile; the manager call here is the same shape (`add={NEEDS_HELP}`) but goes through the standard transition surface for logging consistency.
...
- **Structured logging per transition.** Each manager call emits exactly ONE log line at INFO level via `logger = logging.getLogger("foreman.label_manager")`. Fields (logger `extra=`, NOT f-string interpolation): `issue_url`, `from_labels` (sorted list of strings), `to_labels` (sorted list of strings), `added` (sorted list), `removed` (sorted list), `reason`. The pre-transition + post-transition sets are computed inside the manager so a caller cannot lie about either. A no-op transition (add ∩ current == add AND remove ∩ current == ∅) logs at DEBUG instead of INFO so idempotent retries don't spam.
...
Adding `get_labels` + `set_labels` to `ReconcilerHost` (and `v3_host.py`'s impl + `daemon_host.py`'s underlying `get_issue_labels` already exists, plus a new atomic `set_issue_labels` method) is part of this PR. The existing `host.add_label` / `host.remove_label` methods on `ReconcilerHost` are KEPT for back-compat — the manager doesn't use them, but removing them is a separate cleanup PR (per "Out of scope" of this ticket).
...
1. **Create `packages/foreman/src/foreman/label_manager.py`** with:
   - `LabelWriter` Protocol (`read_current() -> set[str]`, `replace_labels(final: set[str]) -> None`).
...
2. **Write `packages/foreman/tests/test_label_manager.py`** with FakeLabelWriter + unit tests covering:
   ...
   - No-op transitions log at DEBUG, not INFO, and call `replace_labels` zero times.
   - The grep-fence test `test_label_writes_only_go_through_label_manager` that walks `packages/foreman/src/foreman/` and asserts ONLY the allowlisted files contain the literal substrings `add_to_labels`, `remove_from_labels`, `set_labels`. Allowlist: `label_manager.py`, `daemon_host.py`, `git_hosts/github.py`.
...
7. **Migrate `roles/worker.py`** sites: lines 761 (pre-LLM), 1188 (post-LLM outcome), 1323 (crash-revert). Construct `IssueLabelWriter(issue)`; call `transition(...)`. The role's existing `removed_foreman` / `added_foreman` sets become the `remove=` / `add=` kwargs. The `issue.update()` call (current line 1170) is REMOVED — the writer's `read_current` does the cache invalidation. The `lambda: issue.add_to_labels(TERMINAL_BLOCKING_LABEL)` callbacks at lines 674 and 862 become `lambda: LabelManager().transition(IssueLabelWriter(issue), add={Label.NEEDS_HELP}, remove=set(), reason="worker_exception")`.

8. **Migrate `roles/fixer.py`** sites: line 531 (attempt label add — pass `str(f"foreman:fix-attempt-{attempt}")` as a member of `add`), line 790 (outcome). Callbacks at lines 489 and 623 → manager call. Remove the now-redundant `issue.update()` at line 777.

9. **Migrate `roles/reviewer.py`** sites: line 674 (outcome). Callback at line 510 → manager call. Remove the now-redundant `issue.update()` at line 671.

10. **`roles/planner.py`** has no label writes (it stays at `foreman:planning` and writes ZERO labels per the comment at lines 384-398). NO change.
...
13. **Run `just check`**. Confirm ruff clean, mypy clean, full pytest green. Record pre-edit + post-edit test counts in the impl PR body; they must match (no net new test additions OR deletions beyond the new `test_label_manager.py` file, which adds test count rather than changes the existing-test pass count).
...
| `packages/foreman/src/foreman/daemon_host.py` | Add `set_issue_labels(repo, issue_number, labels: set[str])` method next to lines 83-93 (delegates to `issue.set_labels(*sorted(labels))`). `add_issue_label` / `remove_issue_label` STAY (other callers still exist; their cleanup is out of scope). |
...
| `packages/foreman/src/foreman/roles/worker.py` | Migrate 3 `issue.set_labels` sites + 2 exception-callback `issue.add_to_labels` sites to the manager. Remove 2 now-redundant `issue.update()` calls. |
| `packages/foreman/src/foreman/roles/fixer.py` | Migrate 1 `issue.set_labels` site + 1 `issue.add_to_labels(attempt_label)` + 2 exception-callback sites to the manager. Remove 1 redundant `issue.update()`. |
| `packages/foreman/src/foreman/roles/reviewer.py` | Migrate 1 `issue.set_labels` site + 1 exception-callback site to the manager. Remove 1 redundant `issue.update()`. |
...
No expected changes to: `packages/foreman/src/foreman/labels.py` (D1 catalog stays as-is); `reconciler/rules.py` (predicates stay; they become correct as a consequence of the rot being gone); `roles/planner.py` (writes no labels); `daemon_runners.py` (the legacy v1 `merge_spec_pr`/`merge_impl_pr` paths use vestigial labels `foreman:spec-ready` / `foreman:implementing-ready` / `foreman:ready-for-merge` not in the D1 catalog — those are dead code per the D1 introduction and untouched here per "Out of scope").
...
- Removing the `ReconcilerHost.add_label` / `host.remove_label` methods (still used by non-label-mutating call paths — separate cleanup).
