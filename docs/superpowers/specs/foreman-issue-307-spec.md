...
- **Structured logging per transition.** Each manager call emits exactly ONE log line at INFO level via `logger = logging.getLogger("foreman.label_manager")`. Fields (logger `extra=`, NOT f-string interpolation): `issue_url` (sourced from `writer.issue_url`, see Sub-request 1's `LabelWriter` protocol), `from_labels` (sorted list of strings), `to_labels` (sorted list of strings), `added` (sorted list), `removed` (sorted list), `reason`. The pre-transition + post-transition sets are computed inside the manager so a caller cannot lie about either. **A no-op transition is defined as `final == current` AFTER invariants have been applied** — the SAME check Sub-request 1 uses to decide whether to call `replace_labels`. Both decisions (skip-the-write AND DEBUG-vs-INFO logging) use this single post-invariant check. No-op transitions log at DEBUG instead of INFO so idempotent retries don't spam. Worked example for the #303 idempotent-retry case: `current = {foreman:impl-approved, foreman:merging-plan, priority:high}`, `add = {Label.IMPL_APPROVED}`. After applying the QUEUE-strips-IN_FLIGHT invariant, `final = {foreman:impl-approved, priority:high}` ≠ `current`, so this is NOT a no-op — `replace_labels` is called, the INFO log line is emitted, and the WARNING for the stale `merging-plan` strip is also emitted.
...
- **Existing test suite continues to pass with the documented delta.** All currently-passing tests in `packages/foreman/tests/` EXCEPT `tests/test_git_hosts_github.py::test_update_issue_labels_removes_then_adds` (whose target behavior is explicitly removed in this PR per the File-level changes table) continue to pass. The expected net total-test-count delta is `(new tests in test_label_manager.py) − 1`. The Worker records actual pre-edit count + computes + reports the expected post-edit count using this arithmetic; no raw-equality check. Tests that previously inspected `_FakeHost.calls` for `("add_label", ...)` + `("remove_label", ...)` tuples (`tests/reconciler/test_actions.py:135-144` and similar) MAY be updated to assert the new manager-mediated shape (`replace_labels` calls instead, or a recorded `transition()` call) — those updates are mechanical and part of the migration; they add zero net tests.
...
| `packages/foreman/src/foreman/roles/planner.py` | Migrate 1 exception-callback site at line 294. Define a named closure (`_set_needs_help_label`) that performs BOTH the `bound_host.get_issue(...)` fetch AND the `LabelManager().transition(...)` call together; pass `_set_needs_help_label` as `set_needs_help_label=` to `handle_unhandled_role_exception`. The fetch MUST stay INSIDE the closure body so a fetch failure raises inside `handle_unhandled_role_exception`'s per-callable try/except (`roles/__init__.py:154-159`), preserving the runaway-burn `post_comment`-still-fires contract. Full code in Sub-request 10. Happy path (which writes ZERO labels) unchanged. |
