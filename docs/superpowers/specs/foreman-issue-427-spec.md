...
2. **`packages/foreman/src/foreman/roles/worker.py`**: Import `ImplWorktreeRebaseConflictError` and `origin_branch_exists` from `foreman.worktree`. In `_run_worker_core`, after `wt_path = wt_result.path` and before `spec_doc_content = _read_spec_doc_from_branch(...)`, add the rebase block: if `not origin_branch_exists(Path(project.local_clone_path), impl_branch_name)`, call `wt_mgr.rebase_impl_onto_origin(clone_path=..., wt_path=wt_path, base_branch=wt_result.base_branch)`. On `ImplWorktreeRebaseConflictError`, synthesize `WorkerOutput(outcome="incomplete", ...)` with a populated `EscalationComment`, call `post_escalation_comment` and `log_worker_run` (zeros for token/cost/duration_ms/num_turns fields, `baseline_failures_count=0`, `new_failures_count=0`, `duration_seconds=time.monotonic() - start_time`), then return a `WorkerRunResult` early (`pr_url=None`, `final_did_check_pass=False`).
