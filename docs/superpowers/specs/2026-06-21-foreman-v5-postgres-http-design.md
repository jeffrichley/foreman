...
- **Authentication is bind-address + shared secret. Tailnet is the documented happy-path** (resolved 2026-06-21 — Jeff checks in from his laptop, so off-host access is the common case). Recommended setup: `[http].bind = "0.0.0.0:8765"` behind Tailscale's network ACLs, with `[http].shared_secret = "<value>"` gating every mutation (`Authorization: Bearer <secret>`). Localhost-only (`127.0.0.1:8765`) remains supported for single-host operation. Reads are unauthenticated by default on the tailnet (Tailscale ACLs are the perimeter); set `[http].auth_reads_too = true` to gate reads with the shared secret as well. **Bootstrap refuses to start if `bind ≠ 127.0.0.1` AND `shared_secret` is unset** — a loud error, never silent open-mutation exposure.
...
5. **No `Authorization` header on reads by default** (tailnet is the happy-path per the 2026-06-21 resolution). Status data is readable by anything on the tailnet unless `auth_reads_too = true`. Accepted: the Tailscale ACL is the read perimeter, consistent with how the rest of the agent-core fleet is exposed. Mutations always require the shared secret when bound off-localhost; bootstrap refuses to start if `bind ≠ 127.0.0.1` AND `shared_secret` is unset. Loud error, not silent permissiveness.
