...
`bootstrap_cli_context` in `packages/foreman/src/foreman/v4/bootstrap.py` already mints `orch_token` for the clone loop. The webhook coverage check must reside **inside** the existing `if run_startup_clone and active_projects:` guard (bootstrap.py line 104) — that is the only scope where `orch_token` is bound. Add an inner `if config.inbound is not None:` block at the bottom of that guard, after the per-project clone `for` loop, to build a `Github(auth=Auth.Token(orch_token))` client and check each project's hooks. Call `logger.error(...)` for any project missing the inbound hook, `logger.info(...)` for each covered project. When `run_startup_clone=False` (the sandboxed-role-subprocess path, which per the bootstrap docstring "has no PEM to mint with"), this block is skipped alongside the clone loop — no `NameError`, no misleading absence of the check. This matches the "report loudly" directive — an ERROR-level log entry is visible in the daemon's structured log stream and in `docker compose logs`.
...
3. **Add `_check_webhook_coverage` to `doctor.py`:** pure function taking `(config: V4Config, projects: list[ProjectConfig], *, github_factory: Callable[[], Github] | None = None) -> int`. Returns 0 on SKIPPED/OK/WARN, 1 on MISSING. The `github_factory` parameter is a zero-arg callable — injectable for tests; in production `cmd_doctor` always passes a closure that captures the pre-minted token (see sub-request 4), so no default minting logic lives here. `None` is accepted only as a sentinel so tests can also pass the factory positionally via the keyword form. Return SKIPPED (exit 0) immediately when `not config.inbound or not config.inbound.receiver_url` — an absent `[inbound]` block and an empty `receiver_url` are both treated as unconfigured (matching acceptance criterion 3). When `projects` is empty, also return SKIPPED (exit 0) immediately; the function should not assume `projects` is non-empty.

4. **Extend `cmd_doctor` to load config + projects and call the new check:** load config from `FOREMAN_V4_CONFIG` env var; if load fails, print WARN + skip the check. Load projects from `FOREMAN_PROJECTS_PATH`; if load fails, print WARN + skip. If projects loaded successfully but the list is empty, print WARN (no projects to check) and skip the call to `_check_webhook_coverage`. Mint the orchestrator token by constructing `V4IdentityRegistry(apps=config.apps, orchestrator=config.orchestrator, installation_repo=projects[0].repo)` and calling `.get_role_token("orchestrator")` — the module docstring for `identity.py` confirms that any project's repo works for the installation-id lookup in a single-org deployment, so `projects[0].repo` is the correct choice. Pass the pre-minted token as a closure `github_factory=lambda: Github(auth=Auth.Token(orch_token))` to `_check_webhook_coverage` — the factory is zero-arg because the token is already captured. On `GithubException` during minting, treat as WARN + exit 0.

5. **Add startup webhook log to `bootstrap_cli_context`** (`bootstrap.py`): **inside** the `if run_startup_clone and active_projects:` block (bootstrap.py line 104), after the per-project clone `for` loop, add `if config.inbound is not None:` and build a `Github(auth=Auth.Token(orch_token))` client to check each project's hooks. Log `logger.error(...)` per missing project, `logger.info(...)` per covered project. Do **not** reference `orch_token` outside the `if run_startup_clone and active_projects:` guard — that variable is only bound in that scope, and when `run_startup_clone=False` (the sandboxed-role-subprocess path that has no PEM to mint with) there is no way to obtain a fresh token.
