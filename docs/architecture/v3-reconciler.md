...
4. **Lagging-label rules (`advance_label_to_plan_approved_lagging`, `advance_label_to_done`) have a 24h `has_recent` guard** but no test for multi-tick re-fire scenarios. If a manual label revert happened, would the rule re-fire? Probably yes; unverified.
