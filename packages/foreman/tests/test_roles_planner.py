...
async def test_run_planner_dispatches_and_advances_label(
    ...
    # The Planner makes zero label mutations on the happy path. Under
    # v4, ``LabelObservabilityObserver`` owns every ``foreman:*`` write
    # off state transitions, so any label call here would be a regression.
