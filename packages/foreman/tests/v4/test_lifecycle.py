...
# detect the human merge, so the driver must not stop there. The machine
# only truly halts at Done/Failed/NeedsHelp.
...
def test_impl_approved_operator_resume_to_done():
    """foreman#443: ImplApproved auto-detects the human merge and closes
    the originating issue, without any operator set-state intervention.

    Uses a DISTINCT impl PR number (#99) so the spec PR (#42, merged by
    SpecMerging) and the impl PR can be controlled independently.

    Stage 1: first ImplApproved dispatch — impl PR #99 is still open →
             BLOCKED self-loop. The driver does NOT stop at ImplApproved
             (it is no longer a terminal) and continues.
    Stage 2: scripted git flips PR #99 to merged on the second call —
             second ImplApproved dispatch → CLEAN → DoneState.

    Assertions:
    - Final state is Done.
    - foreman never called merge_pr on #99 (the human merged it).
    - The originating issue was closed.
    - The journal shows ImplApproved twice (once BLOCKED, once CLEAN→Done).
    """
    repo = InMemoryTicketRepository()
    ticket = repo.create_ticket(project="p", issue_number=1, now=dt.datetime(2026, 6, 13))

    # Scripted git: PR #42 is the spec PR (starts mergeable, SpecMerging
    # merges it). PR #99 is the impl PR: first poll returns open, second
    # returns merged — simulating the human pulling the merge trigger
    # between two Poller ticks.
    class _ScriptedImplGit(FakeGitProvider):
        def __init__(self) -> None:
            super().__init__()
            self._impl_pr_polls = 0

        def get_pr_state(self, *, project: str, pr_number: int) -> PRState:
            if pr_number == 99:
                self._impl_pr_polls += 1
    ...
    # foreman must NOT have called merge_pr on the impl PR — the human merged.
    assert ("p", 99) not in git.merge_pr_calls
    # The originating issue was closed when ImplApproved detected the merge.
    assert ("p", 1) in git.closed_issues
    # ImplApproved polled exactly twice: first BLOCKED, then CLEAN → Done.
    assert git._impl_pr_polls == 2

    state_order = [r.state_name for r in repo.list_state_instances_for_ticket(ticket.id)]
    # ImplApproved appears twice in the journal.
    assert state_order.count("ImplApproved") == 2
    assert state_order[-1] == "Done"
